...
**awesome-cognis** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
the suite's integration SDK. It maps any tool's JSON into a canonical **Finding** and
forwards it to the platforms that fit the **Dataset / catalog** domain.
...
Once `awesome-cognis` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
awesome-cognis ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
awesome-cognis ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```

Recommended for this domain: **stix, webhook**. The full set is
...
works whatever `awesome-cognis` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="awesome-cognis") for rec in records]   # records = your JSON output
