...
How **awesome-cognis** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `awesome-cognis` lives in the **Dataset / catalog** cluster.
...
| from | relation | to |
|---|---|---|
| `awesome-cognis` | JSON findings compose with | [`awesome-drone-warfare-osint`](https://github.com/cognis-digital/awesome-drone-warfare-osint) |
| `awesome-cognis` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `awesome-cognis` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `awesome-cognis` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the dataset / catalog cluster.** `awesome-cognis` output feeds the next tool:
```bash
awesome-cognis ... --format json > out.json          # this tool's findings (see `awesome-cognis --help`)
awesome-drone-warfare-osint ... < out.json                      # the cluster sibling consumes them
```
...
awesome-cognis ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
awesome-cognis ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
awesome-cognis ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `awesome-cognis`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `awesome-cognis` slots
